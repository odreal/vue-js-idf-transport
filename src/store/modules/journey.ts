...
const journey: Module<{ journeys: Journey[] }, any> = {
  ...
  mutations: {
    add(state, payload: { hStart: Date, hArrive: Date, c02: number, duration: number, step: Array<Step>  }) {
      state.journeys.push({
        id: getRandomId(),
  ...
  actions: {
    ...
    search(context, payload: {firstPoint: String, secondPoint: String}) {
      ...
      function getRoute(): void {
          axios.get(
                    'https://api.navitia.io/v1/coverage/fr-idf/journeys?to='+tmpCoordinate[1].coord.lon+';'+tmpCoordinate[1].coord.lat+'&from='+tmpCoordinate[0].coord.lon+';'+tmpCoordinate[0].coord.lat+'&datetime_represents=arrival&datetime=20191102T110000',
                    {headers: {
                        "Authorization" : "edddfa70-8fbf-4d51-a4c9-e075416c639d"
                      }
                    }
        ...
        response.data.journeys.forEach(element => {
          ...
          element.sections.forEach(section => {
            if(section.display_informations != undefined){
                var tmpObject = {'id': getRandomId(), 'from': section.from.name, 'to': section.to.name,'mode':section.display_informations.commercial_mode, 'code': section.display_informations.code, 'direction': section.display_informations.direction, 'hStart': new Date(parseDate(section.base_departure_date_time)), 'hArrive': new Date(parseDate(section.base_arrival_date_time)) }
                tmpArray.push(tmpObject)
                console.log(tmpArray)
            }
          });
          if(tmpArray.length>0){
            context.commit('add', { hStart: new Date(parseDate(element.departure_date_time)), hArrive: new Date(parseDate(element.arrival_date_time)), duration: element.duration, c02: element.co2_emission.value, step: tmpArray });
          }
        });
      }
    }
